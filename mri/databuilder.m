...
% Obsoleted model check
check_databuilder_model(model);
...
function check_databuilder_model(model)
% Check databuilder model
%

if ~isfield(model, 'event_dir') & ~isfield(model, 'taskdir')
    warning(sprintf(['`event_dir` is not supecified. ', ...
                     'The current default task event data directory is `event`. ', ...
                     'Previously the default was `task`. ', ...
                     'Omitting the directory specification may cause an error or unintended results. ', ...
                     'To avoid that, please specify `event_dir` as below:\n', ...
                     '\n', ...
                     '    %% BIDS compatible task event files (e.g., tsv files):\n', ...
                     '    model.event_dir = ''event'';\n', ...
                     '    builder = databuilder(entry, model);\n', ...
                     '\n', ...
                     '    %% Obsoleted task data files (e.g., mat files):\n', ...
                     '    model.event_dir = ''task'';\n', ...
                     '    builder = databuilder(entry, model);\n', ...
                     '\n']));
end

if ~isfield(model, 'event_file_type') & ~isfield(model, 'taskfiletype')
    warning(sprintf(['`event_file_type is not supecified. ', ...
                     'The current default task event file is a tsv file. ', ...
                     'Previously the default was a mat file. ', ...
                     'Omitting the directory specification may cause an error or unintended results. ', ...
                     'To avoid that, please specify `event_file_type` as below:\n', ...
                     '\n', ...
                     '    %% BIDS compatible task event files (e.g., tsv files):\n', ...
                     '    model.event_file_type = ''tsv'';\n', ...
                     '    builder = databuilder(entry, model);\n', ...
                     '\n', ...
                     '    %% Obsoleted task data files (e.g., mat files):\n', ...
                     '    model.event_file_type = ''mat'';\n', ...
                     '    builder = databuilder(entry, model);\n', ...
                     '\n']));
end
